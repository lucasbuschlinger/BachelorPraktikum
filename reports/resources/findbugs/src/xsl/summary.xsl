...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:param name="PACKAGE.BUGCLASS.LABEL"
               select="'Most Buggy Class in Package with #1 $1:'"/>
            ...
            <body>
                <h1 align="center">
                    <a href="http://findbugs.sourceforge.net">
                        <xsl:value-of select="$SUMMARY.HEADER"/>
                    </a>
                </h1>
                ...
                <h2 align="center">
                    ...
                    <i>
                        <xsl:value-of select="//FindBugsSummary/@timestamp"/>
                    </i>
                </h2>
                <xsl:apply-templates select="//FindBugsSummary"/>
                <br/>
                <p align="center">
                    <font face="{$PAGE.FONT}" size="6">
                        <xsl:value-of select="$PACKAGE.HEADER"/>
                    </font>
                    <br/>
                    <font face="{$PAGE.FONT}" size="4">
                        <i>(<xsl:value-of select="$PACKAGE.SORT.LABEL"/>)
                        </i>
                    </font>
                </p>
                <xsl:for-each select="//FindBugsSummary/PackageStats">
                    <xsl:sort select="@total_bugs" data-type="number"
                              order="descending"/>
    ...
    <xsl:template name="status_table_row">
        ...
        <tr>
            <td align="left">
                <font face="{$PAGE.FONT}" size="{$FONT_SIZE}">
                    <xsl:value-of select="$LABEL"/>
                </font>
            </td>
            <td align="center">
                <font face="{$PAGE.FONT}" color="green" size="{$FONT_SIZE}">
                    <xsl:value-of select="$COUNT"/>
                </font>
            </td>
            <td align="center">
                <font face="{$PAGE.FONT}" color="red" size="{$FONT_SIZE}">
                    <xsl:value-of select="$BUGS"/>
                </font>
            </td>
            <td align="center">
                <font face="{$PAGE.FONT}" color="blue" size="{$FONT_SIZE}">
                    <xsl:choose>
                        <xsl:when test="$COUNT &gt; 0">
                            <xsl:value-of
                                    select="format-number(number($BUGS div $COUNT), $PERCENTAGE.FORMAT)"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of
                                    select="format-number(0, $PERCENTAGE.FORMAT)"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </font>
            </td>
    ...
    <xsl:template name="table_header">
        <tr>
            <th>
                <font face="{$PAGE.FONT}" size="4">
                    <xsl:value-of select="$TABLE.HEADING.TYPE"/>
                </font>
            </th>
            <th>
                <font face="{$PAGE.FONT}" size="4">
                    <xsl:value-of select="$TABLE.HEADING.COUNT"/>
                </font>
            </th>
            <th>
                <font face="{$PAGE.FONT}" size="4">
                    <xsl:value-of select="$TABLE.HEADING.BUGS"/>
                </font>
            </th>
            <th>
                <font face="{$PAGE.FONT}" size="4">
                    <xsl:value-of select="$TABLE.HEADING.PERCENT"/>
                </font>
            </th>
        </tr>
    </xsl:template>
        ...
        <table width="{$TABLE.WIDTH}" border="1" align="center">
            ...
            <xsl:call-template name="status_table_row">
                ...
                <xsl:with-param name="COUNT"
                                select="count(PackageStats/ClassStats[@interface='false' and substring-after(@class,'$')=''])"/>
                <xsl:with-param name="BUGS"
                                select="sum(PackageStats/ClassStats[@interface='false' and substring-after(@class,'$')='']/@bugs)"/>
            </xsl:call-template>
            ...
            <xsl:call-template name="status_table_row">
                ...
                <xsl:with-param name="COUNT"
                                select="count(PackageStats/ClassStats[@interface='true'])"/>
                <xsl:with-param name="BUGS"
                                select="sum(PackageStats/ClassStats[@interface='true']/@bugs)"/>
            </xsl:call-template>
                    ...
                    <font face="{$PAGE.FONT}" size="4">
                        <xsl:call-template name='string_format'>
                            ...
                            <xsl:with-param name="STRING"
                                            select="$TOTAL.PACKAGES.LABEL"/>
                            <xsl:with-param name="SINGLE"
                                            select="$PACKAGE.SINGLE.LABEL"/>
                            <xsl:with-param name="PULURAL"
                                            select="$PACKAGE.PULURAL.LABEL"/>
                        </xsl:call-template>
                    </font>
                </td>
            </tr>
    ...
    <xsl:template name='string_format'>
        ...
        <xsl:variable name="count_str"
                      select="concat(substring-before($STRING,'#1'), $COUNT, substring-after($STRING,'#1'))"/>

        <xsl:choose>
            <xsl:when test="$COUNT &gt; 1">
                <xsl:value-of
                        select="concat(substring-before($count_str,'$1'), $PULURAL, substring-after($count_str,'$1'))"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of
                        select="concat(substring-before($count_str,'$1'), $SINGLE, substring-after($count_str,'$1'))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template match="PackageStats">
        ...
        <h2 align="center">
            <xsl:value-of select="$PACKAGE.LABEL"/>
            <i>
                <font color='green'>
        ...
        <table width="{$TABLE.WIDTH}" border="1" align="center">
            ...
            <xsl:call-template name="status_table_row">
                <xsl:with-param name="LABEL" select="$TABLE.ROW.OUTER"/>
                <xsl:with-param name="COUNT"
                                select="count(ClassStats[@interface='false' and substring-after(@class,'$')=''])"/>
                <xsl:with-param name="BUGS"
                                select="sum(ClassStats[@interface='false' and substring-after(@class,'$')='']/@bugs)"/>
            </xsl:call-template>
            ...
            <xsl:call-template name="status_table_row">
                <xsl:with-param name="LABEL" select="$TABLE.ROW.INTERFACE"/>
                <xsl:with-param name="COUNT"
                                select="count(ClassStats[@interface='true'])"/>
                <xsl:with-param name="BUGS"
                                select="sum(ClassStats[@interface='true']/@bugs)"/>
            </xsl:call-template>
            ...
            <table width="{$TABLE.WIDTH}" border="0" align="center">
                <xsl:variable name="max_bugs">
                    <xsl:for-each select="ClassStats">
                        <xsl:sort select="@bugs" data-type="number"
                                  order="descending"/>
                        ...
                        <font face="{$PAGE.FONT}" size="4">
                            <xsl:call-template name='string_format'>
                                <xsl:with-param name="COUNT"
                                                select="$max_bugs"/>
                                <xsl:with-param name="STRING"
                                                select="$PACKAGE.BUGCLASS.LABEL"/>
                                <xsl:with-param name="SINGLE"
                                                select="$BUGS.SINGLE.LABEL"/>
                                <xsl:with-param name="PULURAL"
                                                select="$BUGS.PULURAL.LABEL"/>
                            </xsl:call-template>
                        </font>
                    </td>
                </tr>
                ...
                <xsl:for-each select="ClassStats">
                    <xsl:if test="@bugs = $max_bugs">
                        <tr>
                            <td>&#160;&#160;&#160;&#160;&#160;&#160;&#160;</td>
                            <td align="left">
                                <font face="{$PAGE.FONT}" color="red" size="4">
                                    <i>
                                        <xsl:value-of select="$package-prefix"/><xsl:value-of
                                            select="@class"/>
                                    </i>
                                </font>
                            </td>
