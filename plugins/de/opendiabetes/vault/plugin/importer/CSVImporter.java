...
public abstract class CSVImporter extends FileImporter {
     ...
     */
    private CsvReader getValidatedCreader(final char delimiter, final InputStream fileInputStream) throws IOException {
        // open file
    ...
    protected boolean processImport(final InputStream fileInputStream, final String filenameForLogging) {
        ...
        try {
            ...
            if (getDelimiter() == 0) { //try to detect the delimiter by try n error
                LOG.log(Level.INFO, "using automatic delimiter detection");
                char[] delimiterList = {',', ';', '\t'};
                for (char delimiter : delimiterList) {
                    creader = getValidatedCreader(delimiter, fileInputStream);
                    if (null != creader) {
                ...
                creader = getValidatedCreader(getDelimiter(), fileInputStream);
            }
